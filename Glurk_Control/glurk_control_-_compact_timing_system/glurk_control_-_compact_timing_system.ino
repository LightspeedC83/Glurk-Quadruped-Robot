...
//function that concatenates various motor control and/or timing functions with time delays in between (will allow for compact motor control sequences in the form of a given array)
//void execute(int time_splice, commands[]){
...
//function that controlls all the motors in the upper group
...
void creep_walk(int timing, int cycle_repeats){ 
  ...
  for (int i=0; i<=cycle_repeats; i++){  
    ...
    // the right legs going into ready position
    f_r_l(90);
    ...
    // preparing the front right leg to pull
    f_r_l(90);
    delay(foot_lift_delay);
    f_r_u(0);
    delay(foot_lift_delay);
    ...
    // pulling back on the right half  
    f_r_u(100);
    b_r_u(0);

    delay(timing);

    // resetting the right half to base position
    b_r_l(90);
    delay(foot_lift_delay);
    b_r_u(55);
    delay(foot_lift_delay);
    ...
    // left legs going into ready position
    f_l_l(90);
    delay(foot_lift_delay);
    f_l_u(100);
    delay(foot_lift_delay);
    ...
    // preparing the left front leg to pull
    f_l_l(90);
    delay(foot_lift_delay);
    f_l_u(0);
    delay(foot_lift_delay);
    ...
    // pulling back on the left half
    f_l_u(100);
    b_l_u(0);

    delay(timing);
    
    // resetting the left half to the base position  
    f_l_l(90);
    delay(foot_lift_delay);
    f_l_u(55);
    delay(foot_lift_delay);
...
// lurch gait algorithm (I made this up)
void lurch_gait(int timing, int cycle_repeats){ 
  int foot_lift_delay = timing/10;
  
  for (int i=0; i<=cycle_repeats; i++){  
    // preparing front right foot
    delay(foot_lift_delay);
    f_r_l(90);
    delay(foot_lift_delay);
    f_r_u(65);
